/**
 * Korean Common Modules Loader
 * Dynamically loads and inserts common page modules for Korean pages
 */

class KoreanCommonModulesLoader {
    constructor() {
        this.rootPath = this.determineRootPath();
        this.modulesPath = this.rootPath + 'includes/common-modules-ko.html?t=' + new Date().getTime();
        this.modules = {};
        this.isLoaded = false;
    }

    determineRootPath() {
        const script = document.currentScript || document.querySelector('script[src*="common-modules-ko.js"]');
        if (script) {
            const src = script.getAttribute('src');
            const match = src.match(/^(.*)js\/common-modules-ko\.js/);
            if (match) {
                return match[1];
            }
        }
    ...
    async loadModules() {
        try {
            ...
            console.log('Korean common modules loaded successfully');
            return true;
        } catch (error) {
            console.error('Error loading Korean common modules:', error);
            return false;
        }
    }
    ...
    async init() {
        ...
        document.dispatchEvent(new CustomEvent('koreanModulesLoaded'));
    }
}

document.addEventListener('DOMContentLoaded', function() {
    const loader = new KoreanCommonModulesLoader();
    loader.init();
});

window.KoreanCommonModulesLoader = KoreanCommonModulesLoader;
