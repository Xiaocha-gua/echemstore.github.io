/**
 * Japanese Common Modules Loader
 * Dynamically loads and inserts common page modules for Japanese pages
 */

class JapaneseCommonModulesLoader {
    constructor() {
        this.rootPath = this.determineRootPath();
        this.modulesPath = this.rootPath + 'includes/common-modules-ja.html?t=' + new Date().getTime();
        this.modules = {};
        this.isLoaded = false;
    }

    determineRootPath() {
        const script = document.currentScript || document.querySelector('script[src*="common-modules-ja.js"]');
        if (script) {
            const src = script.getAttribute('src');
            const match = src.match(/^(.*)js\/common-modules-ja\.js/);
            if (match) {
                return match[1];
            }
        }
    ...
    async loadModules() {
        try {
            ...
            console.log('Japanese common modules loaded successfully');
            return true;
        } catch (error) {
            console.error('Error loading Japanese common modules:', error);
            return false;
        }
    }
    ...
    async init() {
        ...
        document.dispatchEvent(new CustomEvent('japaneseModulesLoaded'));
    }
}

document.addEventListener('DOMContentLoaded', function() {
    const loader = new JapaneseCommonModulesLoader();
    loader.init();
});

window.JapaneseCommonModulesLoader = JapaneseCommonModulesLoader;
