/**
 * Portuguese Common Modules Loader
 * Dynamically loads and inserts common page modules for Portuguese pages
 */

class PortugueseCommonModulesLoader {
    constructor() {
        this.rootPath = this.determineRootPath();
        this.modulesPath = this.rootPath + 'includes/common-modules-pt.html?t=' + new Date().getTime();
        this.modules = {};
        this.isLoaded = false;
    }

    determineRootPath() {
        const script = document.currentScript || document.querySelector('script[src*="common-modules-pt.js"]');
        if (script) {
            const src = script.getAttribute('src');
            const match = src.match(/^(.*)js\/common-modules-pt\.js/);
            if (match) {
                return match[1];
            }
        }
    ...
    async loadModules() {
        try {
            ...
            console.log('Portuguese common modules loaded successfully');
            return true;
        } catch (error) {
            console.error('Error loading Portuguese common modules:', error);
            return false;
        }
    }
    ...
    async init() {
        ...
        document.dispatchEvent(new CustomEvent('portugueseModulesLoaded'));
    }
}

document.addEventListener('DOMContentLoaded', function() {
    const loader = new PortugueseCommonModulesLoader();
    loader.init();
});

window.PortugueseCommonModulesLoader = PortugueseCommonModulesLoader;
