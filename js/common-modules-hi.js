/**
 * Hindi Common Modules Loader
 * Dynamically loads and inserts common page modules for Hindi pages
 */

class HindiCommonModulesLoader {
    constructor() {
        this.rootPath = this.determineRootPath();
        this.modulesPath = this.rootPath + 'includes/common-modules-hi.html?t=' + new Date().getTime();
        this.modules = {};
        this.isLoaded = false;
    }

    determineRootPath() {
        const script = document.currentScript || document.querySelector('script[src*="common-modules-hi.js"]');
        if (script) {
            const src = script.getAttribute('src');
            const match = src.match(/^(.*)js\/common-modules-hi\.js/);
            if (match) {
                return match[1];
            }
        }
    ...
    async loadModules() {
        try {
            ...
            console.log('Hindi common modules loaded successfully');
            return true;
        } catch (error) {
            console.error('Error loading Hindi common modules:', error);
            return false;
        }
    }
    ...
    async init() {
        ...
        document.dispatchEvent(new CustomEvent('hindiModulesLoaded'));
    }
}

document.addEventListener('DOMContentLoaded', function() {
    const loader = new HindiCommonModulesLoader();
    loader.init();
});

window.HindiCommonModulesLoader = HindiCommonModulesLoader;
