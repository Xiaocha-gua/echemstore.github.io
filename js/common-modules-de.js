/**
 * German Common Modules Loader
 * Dynamically loads and inserts common page modules for German pages
 */

class GermanCommonModulesLoader {
    constructor() {
        this.rootPath = this.determineRootPath();
        this.modulesPath = this.rootPath + 'includes/common-modules-de.html?t=' + new Date().getTime();
        this.modules = {};
        this.isLoaded = false;
    }

    determineRootPath() {
        const script = document.currentScript || document.querySelector('script[src*="common-modules-de.js"]');
        if (script) {
            const src = script.getAttribute('src');
            const match = src.match(/^(.*)js\/common-modules-de\.js/);
            if (match) {
                return match[1];
            }
        }
    ...
    async loadModules() {
        try {
            ...
            console.log('German common modules loaded successfully');
            return true;
        } catch (error) {
            console.error('Error loading German common modules:', error);
            return false;
        }
    }
    ...
    async init() {
        ...
        document.dispatchEvent(new CustomEvent('germanModulesLoaded'));
    }
}

document.addEventListener('DOMContentLoaded', function() {
    const loader = new GermanCommonModulesLoader();
    loader.init();
});

window.GermanCommonModulesLoader = GermanCommonModulesLoader;