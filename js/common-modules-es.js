/**
 * Spanish Common Modules Loader
 * Dynamically loads and inserts common page modules for Spanish pages
 */

class SpanishCommonModulesLoader {
    constructor() {
        this.rootPath = this.determineRootPath();
        this.modulesPath = this.rootPath + 'includes/common-modules-es.html?t=' + new Date().getTime();
        this.modules = {};
        this.isLoaded = false;
    }

    determineRootPath() {
        const script = document.currentScript || document.querySelector('script[src*="common-modules-es.js"]');
        if (script) {
            const src = script.getAttribute('src');
            const match = src.match(/^(.*)js\/common-modules-es\.js/);
            if (match) {
                return match[1];
            }
        }
    ...
    async loadModules() {
        try {
            ...
            console.log('Spanish common modules loaded successfully');
            return true;
        } catch (error) {
            console.error('Error loading Spanish common modules:', error);
            return false;
        }
    }
    ...
    async init() {
        ...
        document.dispatchEvent(new CustomEvent('spanishModulesLoaded'));
    }
}

document.addEventListener('DOMContentLoaded', function() {
    const loader = new SpanishCommonModulesLoader();
    loader.init();
});

window.SpanishCommonModulesLoader = SpanishCommonModulesLoader;