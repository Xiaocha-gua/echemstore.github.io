/**
 * Arabic Common Modules Loader
 * Dynamically loads and inserts common page modules for Arabic pages
 */

class ArabicCommonModulesLoader {
    constructor() {
        this.rootPath = this.determineRootPath();
        this.modulesPath = this.rootPath + 'includes/common-modules-ar.html?t=' + new Date().getTime();
        this.modules = {};
        this.isLoaded = false;
    }

    determineRootPath() {
        const script = document.currentScript || document.querySelector('script[src*="common-modules-ar.js"]');
        if (script) {
            const src = script.getAttribute('src');
            const match = src.match(/^(.*)js\/common-modules-ar\.js/);
            if (match) {
                return match[1];
            }
        }
    ...
    async loadModules() {
        try {
            ...
            console.log('Arabic common modules loaded successfully');
            return true;
        } catch (error) {
            console.error('Error loading Arabic common modules:', error);
            return false;
        }
    }
    ...
    async init() {
        ...
        document.dispatchEvent(new CustomEvent('arabicModulesLoaded'));
    }
}

document.addEventListener('DOMContentLoaded', function() {
    const loader = new ArabicCommonModulesLoader();
    loader.init();
});

window.ArabicCommonModulesLoader = ArabicCommonModulesLoader;
