/**
 * French Common Modules Loader
 * Dynamically loads and inserts common page modules for French pages
 */

class FrenchCommonModulesLoader {
    constructor() {
        this.rootPath = this.determineRootPath();
        this.modulesPath = this.rootPath + 'includes/common-modules-fr.html?t=' + new Date().getTime();
        this.modules = {};
        this.isLoaded = false;
    }

    determineRootPath() {
        const script = document.currentScript || document.querySelector('script[src*="common-modules-fr.js"]');
        if (script) {
            const src = script.getAttribute('src');
            const match = src.match(/^(.*)js\/common-modules-fr\.js/);
            if (match) {
                return match[1];
            }
        }
    ...
    async loadModules() {
        try {
            ...
            console.log('French common modules loaded successfully');
            return true;
        } catch (error) {
            console.error('Error loading French common modules:', error);
            return false;
        }
    }
    ...
    async init() {
        ...
        document.dispatchEvent(new CustomEvent('frenchModulesLoaded'));
    }
}

document.addEventListener('DOMContentLoaded', function() {
    const loader = new FrenchCommonModulesLoader();
    loader.init();
});

window.FrenchCommonModulesLoader = FrenchCommonModulesLoader;